...
class DijkstrasShortestPathAdjacencyList:
    ...
    # To break ties (minimum dist to vertex) in the heap, we keep a 
    ...
    def _dijkstra_internal(self, start:int, end:int|None):
        ...
        while len(pq) != 0:
            node_score, _, node_id = heapq.heappop(pq)
            ...
            for edge in edges:
                ...
                if new_dist < self.dist[edge.to]:
                    ...
                    heapq.heappush(pq, (self.dist[edge.to], self._counter, edge.to))
                    self._counter += 1
